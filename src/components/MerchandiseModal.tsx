...
export default function MerchandiseModal({
  ...
  const getValidOptions = (value: unknown): string[] => {
  ...
  const handleSelectItem = (item: MerchandiseItem) => {
    const validSizes = getValidOptions(item.sizes);
    const validColors = getValidOptions(item.colors);
  ...
  const handleAddToCart = () => {
    ...
    const validSizes = getValidOptions(selectedItem.sizes);
    const validColors = getValidOptions(selectedItem.colors);

    const needsSize = validSizes.length > 0;
    const needsColor = validColors.length > 0;
    ...
    onAddToCart(
      selectedItem,
      needsSize ? selectedSize : 'N/A',
      needsColor ? selectedColor : 'N/A',
      quantity,
      discountedPrice
    );
  ...
  const validSizes = selectedItem ? getValidOptions(selectedItem.sizes) : [];
  const validColors = selectedItem ? getValidOptions(selectedItem.colors) : [];

  const allImages = selectedItem
    ? [
        ...(selectedItem.image_url ? [selectedItem.image_url] : []),
        ...((selectedItem.gallery_images || []).filter(Boolean)),
      ]
    : [];
          ...
          <div className="p-8 max-h-[calc(100vh-200px)] overflow-y-auto">
            {!selectedItem ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {items.map((item) => {
                  const itemSizes = getValidOptions(item.sizes);
                  const itemColors = getValidOptions(item.colors);
                  const hasOptions = itemSizes.length > 0 || itemColors.length > 0;
                        ...
                        {item.stock_quantity > 0 ? (
                          <div className="mt-3 text-center">
                            <button className="w-full bg-teal-500 text-white py-2 rounded-lg font-medium hover:bg-teal-600 transition-colors">
                              {hasOptions ? 'Select Options' : 'View Details'}
                            </button>
                          </div>
                        ) : (
